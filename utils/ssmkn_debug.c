...
void validate_devices( int logical, int bdid, int bus, int physical)
{
	...
	printf("DEBUG: validate_devices: %x\n", bdid);
	...
	printf("DEBUG: validate_devices: number of devices %d\n", number_of_devices);
	...
	for (device=minor_start; device<(minor_start + DEVPERBOARD); device++)
	{
		...
		printf("DEBUG: validate_devices: device loop %d\n", device);

		rdev = makedev(tty_major, m);
		printf("DEBUG: validate_devices: rdev %x\n", rdev);
		printf("DEBUG: validate_devices: physical %x state %x\n", 
			physical,
		    	reported.ctlr_array[physical].cnfg_state);
		    ...
		    (reported.ctlr_array[physical].cnfg_state == BOARDISGOODTOGO))
		{
			if (rdev!= 0){
				...
				sprintf(devname,"%s%c%d", name, 
					mux + 'a',jack + 1);
				printf("DEBUG: validate_devices: devname %s\n",
						devname);
...
void recreate_device( char *name, dev_t dev_id)
{
	...
	printf("DEBUG: recreate_device: %s %x\n", name, dev_id);

	if (!lost && (buffer.st_rdev == dev_id)) {
		printf("DEBUG: recreate_device: !lost\n");
		;
	} else{
		if (!lost){
			unlink(name);
		}
		printf("DEBUG: recreate_device: mknod %s %x %x\n", name, modebits, dev_id);
